...
class PublisherMenu {
    ...
    static goToBasicInfoByURL(apiID){
        ...
        cy.wait('@documents',{timeout: Cypress.config("defaultCommandTimeout")}).its('response.statusCode').should('equal', 200)
    ...
    static goToDeploymentsByURL(apiID){
        ...
        cy.wait('@deployments',{timeout: Cypress.config("defaultCommandTimeout")}).its('response.statusCode').should('equal', 200)
        PublisherComonPage.waitUntillLoadingComponentsExit();
    }
    static goTolifecycleByURL(apiID){
        ...
        cy.wait('@revisions',{timeout: Cypress.config("defaultCommandTimeout")}).its('response.statusCode').should('equal', 200)
        PublisherComonPage.waitUntillLoadingComponentsExit();
    }
    static goToAPIDefinitionByUI(apiID){
        ...
        cy.wait('@swagger',{timeout: Cypress.config("defaultCommandTimeout")}).its('response.statusCode').should('equal', 200)
        PublisherComonPage.waitUntillLoadingComponentsExit();
    }
}
