...
class UsersManagementPage {
    ...
    getSearchUsershButton(){
        return cy.get('input[value="Search Users"]')
    }

    getNoMatchingUsersFoundDialogBox_OKButton(){
        return cy.get("div.ui-dialog-buttonset > button.ui-button")
    }

    getNoMatchingUsersFoundDialogBox_MessageInfoDiv(){
        return cy.get('#messagebox-info')
    }
    getNoMatchingUsersFoundDialogBox_MessageInfoDivSelectorOnly(){
        return '#messagebox-info'
    }
    getDeleteButtonOfUser(userName){
        return cy.get(`[onClick="deleteUser(\\'${userName}\\')"]`)
    }
    getDeleteDialogYesButton(){
