...
Cypress.Commands.add('portalLogin', (username, password, portal, tenant = 'carbon.super') => {
    ...
    if (portal === 'devportal') {
        ...
        cy.wait(3000)
        cy.get('#itest-devportal-sign-in', { timeout: Cypress.config("defaultCommandTimeout") })
          .wait(3000)
          .click({ force: true });
    }
    ...
    const tryLogin = (attemptIndex = 0) => {
        ...
        cy.get('[data-testid=login-page-username-input]', { timeout: Cypress.config("defaultCommandTimeout") })
            .should('be.visible')
            .clear()
            .type(cred.username);
        ...
        const waitForAuthOutcome = (remainingPolls = 15) => {
            cy.location('href', { timeout: Cypress.config("defaultCommandTimeout") }).then((href) => {
                if (href.includes(`/${portal}`)) {
                    cy.url({ timeout: Cypress.config("defaultCommandTimeout") }).should('contains', `/${portal}`);
                    return;
                }
...
Cypress.Commands.add('addNewTenant', (tenant = 'wso2.com', username = 'admin', password = 'admin') => {
    ...
    cy.get('#buttonRow .button', { timeout: Cypress.config("defaultCommandTimeout") });
    cy.get('#domain').click();
...
Cypress.Commands.add('addNewUser', (name = 'newuser', roles = [], password = 'test123') => {
    ...
    cy.get('input[name="username"]', { timeout: Cypress.config("defaultCommandTimeout") }).type(name);
...
Cypress.Commands.add('deleteApi', (name, version) => {
    ...
    cy.wait('@getApis', { timeout: Cypress.config("defaultCommandTimeout") }).then(() => {
...
Cypress.Commands.add('createAnAPI', (name, type = 'REST') => {
    ...
    cy.visit(`/publisher/apis`)
    cy.get('#itest-rest-api-create-menu', { timeout: Cypress.config("defaultCommandTimeout") });
...
Cypress.Commands.add('createAPIByRestAPIDesign', (name = null, version = null, context = null) => {
    ...
    cy.get(`#${apiName}`, { timeout: Cypress.config("defaultCommandTimeout") }).click();
...
Cypress.Commands.add('createAndPublishAPIByRestAPIDesign', (name = null, version = null, context = null) => {
    ...
    cy.visit(`/publisher/apis/create/rest`);
    cy.get('#itest-id-apiname-input', { timeout: Cypress.config("defaultCommandTimeout") }).type(apiName);
    cy.get('#itest-id-apicontext-input').click();
    cy.get('#itest-id-apicontext-input').type(apiContext);
    cy.get('#itest-id-apiversion-input').click();
    cy.get('#itest-id-apiversion-input').type(apiVersion);
    ...
    cy.get('#itest-api-name-version', { timeout: Cypress.config("defaultCommandTimeout") }).should('be.visible');
    cy.get('#itest-api-name-version').contains(apiVersion);
})
...
Cypress.Commands.add('addBusinessInfo', (businessOwnerName, businessOwnerEmail,
...
Cypress.Commands.add('addProperty', (name, value, ifSendToDevPortal) => {
    ...
    cy.wait(2000);
    cy.get('#property-name', { timeout: Cypress.config("defaultCommandTimeout") }).focus().type(name);
...
Cypress.Commands.add('createGraphqlAPIfromFile', (name, version, context, filepath) => {
    ...
    cy.get('[data-testid="browse-to-upload-btn"]').wait(1000)
    ...
    cy.get('[data-testid="itest-create-graphql-api-button"]',)
        .click({ force: true, timeout: Cypress.config("defaultCommandTimeout") });
...
Cypress.Commands.add('modifyGraphqlSchemaDefinition', (filepath) => {
    ...
    uploadedDefinitionPanel = cy.get('[data-testid="uploaded-list-graphql"]').get('li')
        .get('[data-testid="uploaded-list-content-graphql"]')
...
Cypress.Commands.add('createLocalScope', (name, displayname = 'sample display name',
    description = 'sample description', roles = []) => {

    cy.get('h3', { timeout: Cypress.config("defaultCommandTimeout") }).contains("Create New Scope", { timeout: 30000 });
    cy.wait(1000);
...
Cypress.Commands.add('createAPIWithoutEndpoint', (name = null, version = null, type = 'REST') => {
    ...
    cy.visit(`/publisher/apis`);
    cy.get('[data-testid="itest-create-api-button"]', { timeout: Cypress.config("defaultCommandTimeout") });
    cy.get('#itest-rest-api-create-menu').click();
    cy.get('#itest-id-landing-rest-create-default').click();
    cy.get('#itest-id-apiname-input').type(apiName);
    cy.get('#itest-id-apicontext-input').click();
...
Cypress.Commands.add('createAndPublishApi', (apiName = null) => {
    ...
    cy.get('#itest-rest-api-create-menu', { timeout: Cypress.config("defaultCommandTimeout") }).click();
...
Cypress.Commands.add('viewThirdPartyApi', (apiName = null) => {
    cy.get(`[area-label="Go to ${apiName}"]`, { timeout: Cypress.config("defaultCommandTimeout") }).click();
    ...
    cy.get('#advAuthHeader', { timeout: Cypress.config("defaultCommandTimeout") }).should('exist');
...
Cypress.Commands.add('createApplication', (applicationName, perTokenQuota, applicationDescription = null) => {
    ...
    cy.get("#itest-info-bar-application-name", { timeout: Cypress.config("defaultCommandTimeout") })
...
Cypress.Commands.add('deleteApplication', (applicationName) => {
    cy.get("#itest-link-to-applications", { timeout: Cypress.config("defaultCommandTimeout") }).click();
    cy.get('table', { timeout: Cypress.config("defaultCommandTimeout") }).get('tbody')
...
Cypress.Commands.add('createAPIByRestAPIDesignAndSearch', (name = null, version = null, context = null) => {
    ...
    cy.get(`#${apiName}`, { timeout: Cypress.config("defaultCommandTimeout") }).parent().trigger('mouseover').click();
