...
describe("Add API Categories and assign via publisher portal", () => {
    ...
    it("Add API Categories and assign via publisher portal", {
        ...
        cy.get('[data-testid="API Categories"]', { timeout: Cypress.config("defaultCommandTimeout") }).click();
        ...
        Utils.addAPI({}).then((apiId) => {
            ...
            cy.get('#APICategories-autocomplete', { timeout: Cypress.config("defaultCommandTimeout") }).click();
    ...
    afterEach(function () {
        if (testApiId) {
            Utils.deleteAPI(testApiId).then(() => {
                ...
                cy.wait(7000);
                cy.get('[data-testid="MuiDataTableBodyCell-4-0"] > div > div > span:nth-child(2)', { timeout: Cypress.config("defaultCommandTimeout") })
                    .should('be.visible')    
                    .click();
                ...
                //cy.get('div[role="status"]', {timeout: Cypress.config("defaultCommandTimeout")}).should('have.text', 'API Category deleted successfully');
            });
        }
    })
