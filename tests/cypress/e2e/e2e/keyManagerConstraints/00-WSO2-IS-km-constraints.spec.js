...
describe('Key Manager Constraints (Admin & DevPortal)', () => {
    ...
    it('1. should create a WSO2 IS Key Manager in Admin portal and set constraints', () => {
        ...
        cy.get('[data-testid="Key Managers"]', { timeout: Cypress.config("defaultCommandTimeout") }).click();
    
        // Click Add Key Manager
        cy.get('[data-testid="add-key-manager-button"]', { timeout: Cypress.config("defaultCommandTimeout") }).click();

        // Fill Basic Info
        cy.get('input[name="name"]', { timeout: Cypress.config("defaultCommandTimeout") }).type(kmName);
    ...
    it('2. should enforce the configured constraints in the DevPortal', () => {
        ...
        cy.get('#production-keys-oauth', { timeout: Cypress.config("defaultCommandTimeout") }).click();
        cy.get(`#${kmName}`, { timeout: Cypress.config("defaultCommandTimeout") })
            .scrollIntoView({ offset: { top: -100, left: 0 } })
            .should('be.visible')
            .click({ force: true });
        ...
        const verifyMaxConstraint = (inputId, maxAllowedValue) => {
            ...
            cy.get(inputSelector, { timeout: Cypress.config("defaultCommandTimeout") })
    ...
    after(() => {
        ...
        cy.get('td > div', { timeout: Cypress.config("defaultCommandTimeout") }).then(($els) => {
