        ...
        function testAPISecurityEnabled() {
            Utils.addAPI({ name: apiName, version: apiVersion }).then((apiId) => {
                if (apiId !== undefined) {
                    ...
                    cy.get('#itest-api-details-api-config-acc', { timeout: Cypress.config("defaultCommandTimeout") }).click();
                    ...
                    cy.get('button[aria-label="disable security for all"]', { timeout: Cypress.config("defaultCommandTimeout") }).should('exist');

                    //deploy API
                    cy.get("#left-menu-itemdeployments").click();
                    cy.wait(2000);
                    cy.get("#deploy-btn", { timeout: Cypress.config("defaultCommandTimeout") }).should('not.have.class', 'Mui-disabled').click({ force: true });
                    cy.contains('div[role="button"]', 'Successfully Deployed').should('exist');

                    cy.get("#left-menu-itemlifecycle").click();
                    cy.wait(2000);
                    cy.get('[data-testid="Deploy as a Prototype-btn"]', { timeout: Cypress.config("defaultCommandTimeout") }).click();
                    ...
                    cy.get('[data-testid="itest-link-to-apis"]', { timeout: Cypress.config("defaultCommandTimeout") }).click();

                    cy.get('input[placeholder="Search APIs & MCP Servers"]').click().type(apiName + "{enter}");
                    cy.get('table > tbody > tr', { timeout: Cypress.config("defaultCommandTimeout") }).get(`[area-label="Go to ${apiName}"]`).should('contain.text', 'PRE-RELEASED');
                    cy.get('table > tbody > tr', { timeout: Cypress.config("defaultCommandTimeout") }).get(`[area-label="Go to ${apiName}"]`).click();
                    ...
                    cy.get("#left-menu-test", { timeout: Cypress.config("defaultCommandTimeout") }).click();

                    cy.intercept('**/applications/').then((res) => {
                        // Check if the application exists
                        cy.get("#selected-application", { timeout: Cypress.config("defaultCommandTimeout") }).should('exist');
                    });

                    //it takes some time to generate the key
                    cy.intercept('**/generate-token').as('getToken');

                    cy.get('#gen-test-key', { timeout: Cypress.config("defaultCommandTimeout") }).click();

                    cy.wait('@getToken', { timeout: Cypress.config("defaultCommandTimeout") }).its('response.statusCode').should('eq', 200);

                    cy.get('.opblock-summary-get > .opblock-summary-control', { timeout: Cypress.config("defaultCommandTimeout") }).click();
                    cy.get('.try-out__btn').click();
                    cy.get('.execute').click();
                    cy.contains('.live-responses-table .response > .response-col_status', '200', { timeout: Cypress.config("defaultCommandTimeout") }).should('exist');
