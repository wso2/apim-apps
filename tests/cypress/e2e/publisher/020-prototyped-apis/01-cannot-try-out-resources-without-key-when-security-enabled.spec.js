...
describe("prototype apis with security enabled", () => {
    ...
    it.only("try out resources enabling the security without credentials", {
        ...
        Utils.addAPI({name: apiName, version: apiVersion}).then((apiId) => {
            ...
            cy.contains('button', "Try Out", { timeout: Cypress.config("defaultCommandTimeout") }).click();
            ...
            //cy.contains('.live-responses-table .response > .response-col_status','401',  {timeout: Cypress.config("defaultCommandTimeout")}).should('exist');
            cy.wait(5000)
            cy.wait('@getExecute').then(() => {
                ...
                //cy.contains('.live-responses-table .response > .response-col_status','401',  {timeout: Cypress.config("defaultCommandTimeout")}).should('exist');
                cy.get('.live-responses-table .response > td.response-col_status',{timeout: Cypress.config("defaultCommandTimeout")}).should("contain.text",'401')
