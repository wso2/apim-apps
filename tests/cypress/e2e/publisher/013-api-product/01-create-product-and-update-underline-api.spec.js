...
describe("Mock the api response and test it", () => {
    ...
    it("Mock the api response and test it", {
        ...
        cy.visit(`/publisher/apis/create/openapi`, { timeout: Cypress.config("defaultCommandTimeout") }).wait(5000)
        ...
        cy.wait(3000);
        cy.get('#itest-id-apiversion-input', { timeout: Cypress.config("defaultCommandTimeout") });
        cy.document().then((doc) => {
            ...
            cy.url().should('contains', 'overview').then(url => {
                ...
                // validate
                cy.get('#itest-api-name-version', { timeout: Cypress.config("defaultCommandTimeout") });
                ...
                cy.wait('@swaggerGet', { timeout: Cypress.config("defaultCommandTimeout") }).then(() => {
                    ...
                    cy.url().should('contains', 'overview').then(urlProduct => {
                        ...
                        const target = '/test';
                        cy.get('#operation-target', { timeout: Cypress.config("defaultCommandTimeout") });
                        ...
                        cy.get('#resource-wrapper', { timeout: Cypress.config("defaultCommandTimeout") }).wait(2000);
