...
describe("Create GraphQl API from file", () => {
  ...
  it("Verify GraphQl API Capabilities", () => {
    ...
    cy.createGraphqlAPIfromFile(apiName, apiVersion, apiContext, filepath).then(value => {
      ...
      //localscopes
      cy.get('#left-menu-itemLocalScopes', { timeout: Cypress.config("defaultCommandTimeout") }).should('have.attr', 'href')
        .then((href) => {
          ...
          cy.get('#left-menu-itemLocalScopes').should('have.attr', 'href')
            .then((href) => {
              ...
              cy.get('#left-menu-operations', { timeout: Cypress.config("defaultCommandTimeout") }).click();
              ...
              cy.location('pathname').then((pathName) => {
                ...
                cy.get('#deploy-btn', { timeout: Cypress.config("defaultCommandTimeout") })
                ...
                //publish
                cy.get("#left-menu-overview", { timeout: Cypress.config("defaultCommandTimeout") }).click();
                cy.get('[data-testid="publish-state-button"]', { timeout: Cypress.config("defaultCommandTimeout") })
                ...
                cy.get('[data-testid="itest-link-to-apis"]',
                  { timeout: Cypress.config("defaultCommandTimeout") }).click();

                cy.visit(`/devportal/apis/${apiId}/overview`)

                //should contain two urls : HTTP URL and Websocket URL
                cy.get('#gateway-envirounment', { timeout: Cypress.config("defaultCommandTimeout") })
                ...
                cy.intercept('**/applications/').then((res) => {
                  ...
                  cy.get("#selected-application", { timeout: Cypress.config("defaultCommandTimeout") })
                    .should('exist');
                });

                cy.intercept('**/generate-token').as('getToken');
                cy.get('#gen-test-key', { timeout: Cypress.config("defaultCommandTimeout") }).click();
                cy.wait('@getToken', { timeout: Cypress.config("defaultCommandTimeout") })
