...
describe("publisher-019-00 : Verify that read only user cannot create updte api", () => {
    ...
    const initEnvironement = () => {
        ...
        UsersAndRoles.addNewUserAndUpdateRoles(creatorPublisher,
            ['Internal/publisher', 'Internal/creator'], creatorpublisherPassword);
    ...
    it("Verify Configurations are in Read only mode", {
        ...
        cy.get('a').get(`[aria-label="${apiName} Thumbnail"]`, { timeout: Cypress.config("defaultCommandTimeout") }).click();
        cy.get('#itest-api-details-portal-config-acc').click();
        ...
        cy.get('#left-menu-itemdocuments').click();
        cy.get('[data-testid="add-document-btn"]', { timeout: Cypress.config("defaultCommandTimeout") })
            .get('[aria-disabled="true"]').should('exist');

        //6. should not be able to comments
        cy.get('#left-menu-itemcomments').click();
        cy.get('#standard-multiline-flexible', { timeout: Cypress.config("defaultCommandTimeout") }).should('be.disabled');
        ...
        cy.get('#add-new-property', { timeout: Cypress.config("defaultCommandTimeout") }).should('be.disabled');
