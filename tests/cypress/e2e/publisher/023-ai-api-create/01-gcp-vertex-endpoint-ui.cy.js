...
describe("GCP (Vertex AI) endpoint UI", () => {
    ...
    it("Edits the primary endpoint and then adds a sandbox endpoint for GCP (Vertex AI)", function () {
        ...
        getVisibleOptions().then((providerOptions) => {
            ...
            createVertexAIAPI(vertexProvider).then((apiId) => {
                // === Flow 1: edit the primary (default production) endpoint ===
                // A freshly created AI API already has a default production endpoint whose URL is the seeded
                // template (unfilled {region}/{project_id}). The real user journey starts here: open the
                // Endpoints page and edit that primary endpoint rather than adding a brand-new one.
                cy.visit(`${Utils.getAppOrigin()}/publisher/apis/${apiId}/endpoints`,
                    { timeout: Cypress.env("largeTimeout") });
                publisherComonPage.waitUntillPublisherLoadingSpinnerExit();

                // The default production endpoint is listed as the primary endpoint; open it via its edit
                // (pencil) action on the card.
                cy.contains("Default Production Endpoint", { timeout: Cypress.env("largeTimeout") })
                    .should("be.visible")
                ...
                // 2. The service-account key section is present (the key is optional - keyless is the
                // fallback). Expand it and confirm the upload control + keyless hint.
                ...
                // 3. The user resolves the templated URL by filling the region + project ID fields (the
                // builder writes them into the URL). Keyless: no key uploaded. Save and return to the list.
                ...
                // The primary endpoint now shows the resolved URL - the region is written into the locations
                // path, so the {project_id}/{region} placeholders are gone.
                cy.contains(`locations/${REGION}`, { timeout: Cypress.env("largeTimeout") }).should("exist");

                // === Flow 2: add a sandbox endpoint ===
                cy.contains("Add New Endpoint", { timeout: Cypress.env("largeTimeout") })
                    .click({ force: true });
                publisherComonPage.waitUntillPublisherLoadingSpinnerExit();
                ...
                // The GCP builder is rendered for the sandbox endpoint too.
                cy.get("#gcp-project-input", { timeout: Cypress.env("largeTimeout") }).should("be.visible");

                // Key-upload path: name + region + project ID + a valid service-account key JSON, then save.
                cy.get("#name").clear({ force: true }).type("gcp-sandbox-endpoint", { force: true });
                cy.get("#gcp-region-input").clear({ force: true }).type(`${REGION}{esc}`, { force: true });
                cy.get("#gcp-project-input").clear({ force: true }).type(PROJECT_ID, { force: true });
                cy.contains("GCP Service Account Key").click({ force: true });
                ...
                // The new sandbox endpoint is listed.
                cy.contains("gcp-sandbox-endpoint", { timeout: Cypress.env("largeTimeout") }).should("exist");
