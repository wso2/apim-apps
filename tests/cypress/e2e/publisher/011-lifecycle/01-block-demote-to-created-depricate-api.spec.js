...
describe("Lifecycle changes", () => {
    ...
    it.only("Block demote retire api", {
        ...
        Utils.addAPIWithEndpoints({ name: apiName, version: apiVersion }).then((apiId) => {
            ...
            cy.get('#itest-api-details-portal-config-acc', {timeout: Cypress.config("defaultCommandTimeout")}).click();
            ...
            cy.contains("Create revisions and deploy in Gateway Environments", {timeout: Cypress.config("defaultCommandTimeout")})
            // Going to lifecycle page
            cy.get('#left-menu-itemlifecycle').click();

            // Publishing
            cy.wait(3000);
            cy.get('[data-testid="Publish-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).click();

            cy.wait(2000);
            cy.contains("LC has changed from CREATED to PUBLISHED", {timeout: Cypress.config("defaultCommandTimeout")})
            cy.get('button[data-testid="Demote to Created-btn"]').should('exist');
            cy.contains("CREATED", {timeout: Cypress.config("defaultCommandTimeout")})
            cy.get('#left-menu-itemlifecycle').click();
            cy.wait(2000);
            ...
            // Block
            cy.get('[data-testid="Block-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).click();
            cy.wait(2000);
            ...
            cy.get('button[data-testid="Re-Publish-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).click();
            cy.get('button[data-testid="Deprecate-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).should('exist');
            cy.get('#left-menu-itemlifecycle').click();
            cy.wait(2000);

            // Deprecate

            cy.get('button[data-testid="Deprecate-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).click();
            cy.get('#itest-id-conf', {timeout: Cypress.config("defaultCommandTimeout")}).contains('DEPRECATE').click();

            cy.get('button[data-testid="Retire-btn"]').should('exist');
            cy.wait(2000);
            cy.get('button[data-testid="Retire-btn"]', {timeout: Cypress.config("defaultCommandTimeout")}).click(); 
            cy.get('#itest-id-conf', {timeout: Cypress.config("defaultCommandTimeout")}).contains('RETIRE').click();
        });
    });
    afterEach(() => {
