    ...
    it.only("Subscribe to API", {
        ...
        Utils.addAPIWithEndpoints({ name: apiName, version: apiVersion, context: apiContext }).then((apiId) => {
            ...
            Utils.publishAPI(apiId).then(() => {
                ...
                cy.get('#wizard-next-1-btn', {timeout: Cypress.config("defaultCommandTimeout")})
                cy.get('#wizard-next-1-btn').click();
            
                cy.get('#wizard-next-2-btn', {timeout: Cypress.config("defaultCommandTimeout")});
                cy.get('#wizard-next-2-btn').click();
            
                cy.intercept('GET','**/oauth-keys').as('oauthKeys');
                cy.wait('@oauthKeys', {timeout: Cypress.config("defaultCommandTimeout")}).then(() => {
                    cy.get('#wizard-next-3-btn', {timeout: Cypress.config("defaultCommandTimeout")});
