    ...
    it.only("Subscribe unsubscribe to app from application view", () => {
        ...
        Utils.addAPIWithEndpoints({ name: apiName, version: apiVersion, context: apiContext }).then((apiId) => {
            ...
            Utils.publishAPI(apiId).then(() => {
                ...
                cy.get(`#itest-application-list-table td a`, {timeout: Cypress.config("defaultCommandTimeout")}).contains(appName).click();
                ...
                cy.wait('@apiGetFirst', {timeout: Cypress.config("defaultCommandTimeout")}).then(() => {
                    cy.wait(2000)
