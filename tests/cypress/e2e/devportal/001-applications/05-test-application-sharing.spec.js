...
describe("Invoke API Product", () => {
    ...
    it("Test Application Sharing", {
        ...
        cy.location('pathname').then((pathName) => {
            ...
            cy.get('#itest-devportal-sign-in', { timeout: Cypress.config("defaultCommandTimeout") }).click({ force: true });
            cy.get('#registerLink', { timeout: Cypress.config("defaultCommandTimeout") }).click();
            ...
            cy.get('input[name="http://wso2.org/claims/givenname"]', { timeout: Cypress.config("defaultCommandTimeout") }).type(user1);
            ...
            cy.get('#registrationSubmit').click();
            cy.get('button.cancel', { timeout: Cypress.config("defaultCommandTimeout") }).click()

            //Creating user 2
            cy.get('#registerLink').click();
            cy.get('#username', { timeout: Cypress.config("defaultCommandTimeout") }).type(user2);
            cy.get('#registrationSubmit').click();
            ...
            cy.get('#application-name', { timeout: Cypress.config("defaultCommandTimeout") }).click();
            ...
            cy.get('body').then(($body) => {
                if ($body.text().includes(appName)) {
                    cy.contains(appName, { timeout: Cypress.config("defaultCommandTimeout") }).click();
                    cy.location('pathname').then((pathName) => {
                        const pathSegments = pathName.split('/');
                        ...
                        cy.visit(`/devportal/applications`);
                        cy.get(`#delete-${appName}-btn`, { timeout: Cypress.config("defaultCommandTimeout") });
