            ...
            <Grid item xs={12}>
                {modelList.length === 1 ? (
                    <FormControl size='small' fullWidth sx={{ mb: 1.5 }}>
                        <InputLabel id={`model-label-${ruleId}`}>
                            <FormattedMessage
                    ...
                    <>
                        <FormControl size='small' fullWidth sx={{ mb: 1.5 }}>
                            <InputLabel id={`vendor-label-${ruleId}`}>
                                <FormattedMessage
                ...
                <FormControl size='small' fullWidth sx={{ mb: 1.5 }}>
                    <InputLabel id={`endpoint-label-${ruleId}`}>
                        <FormattedMessage
...
const ContentBasedRouter: FC<ContentBasedRouterProps> = ({
    ...
    const handleUpdate = (
        ...
        setConfig((prevConfig) => ({
            ...prevConfig,
            [env]: prevConfig[env].map((item) => (item.id === ruleId ? updatedRule : item)),
        }));
    }

    const handleDelete = (env: 'production' | 'sandbox', ruleId: string) => {
        setConfig((prevConfig) => ({
            ...prevConfig,
            [env]: prevConfig[env].filter((rule) => rule.id !== ruleId),
        }));
    }
            ...
            <Grid item xs={12}>
                {endpointError && (
                    <Alert severity='error' sx={{ mb: 2 }}>
                        <FormattedMessage
                            id='Apis.Details.Policies.CustomPolicies.ContentBasedRouter.error.endpoints'
                            defaultMessage='Failed to load endpoints'
                        />
                    </Alert>
                )}
                {modelError && (
                    <Alert severity='error' sx={{ mb: 2 }}>
                        <FormattedMessage
                            id='Apis.Details.Policies.CustomPolicies.ContentBasedRouter.error.models'
                                ...
                                {config.production.map((rule) => (
                                    <RoutingRuleCard
                                        ...
                                        onUpdate={(updatedRule) =>
                                            handleUpdate('production', rule.id, updatedRule)
                                        }
                                        onDelete={() => handleDelete('production', rule.id)}
                                    />
                                ))}
                            </>
                        )}
                            ...
                            <>
                                ...
                                {config.sandbox.map((rule) => (
                                    <RoutingRuleCard
                                        ...
                                        onUpdate={(updatedRule) =>
                                            handleUpdate('sandbox', rule.id, updatedRule)
                                        }
                                        onDelete={() => handleDelete('sandbox', rule.id)}
                                    />
                                ))}
                            </>
                        )}
