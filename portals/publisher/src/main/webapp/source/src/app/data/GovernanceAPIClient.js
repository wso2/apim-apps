...
class GovernanceAPIClient {
     ...
     * @param {{}} args - Accept as an optional argument for GovernanceAPIClient constructor.Merge the given args with
     *  default args.
     * @returns {GovernanceAPIClient}
     */
    constructor(environment, args = {}) {
        ...
        const promisedResolve = SwaggerClient.resolve({
            url: Utils.getGovernanceSwaggerURL(),
            requestInterceptor: (request) => {
                request.headers.Accept = 'text/yaml';
            },
        });
        GovernanceAPIClient.spec = promisedResolve;
     ...
     * @returns {GovernanceAPIClient} an instance of GovernanceAPIClient class
     */
    get client() {
        return this._client;
    }
    ...
    _getResponseInterceptor() {
        return (data) => {
            if (data.headers.etag) {
                GovernanceAPIClient.addETag(data.url, data.headers.etag);
            }
    ...
    _getRequestInterceptor() {
        return (request) => {
            ...
            if (GovernanceAPIClient.getETag(request.url)
                && (request.method === 'PUT' || request.method === 'DELETE' || request.method === 'POST')) {
                request.headers['If-Match'] = GovernanceAPIClient.getETag(request.url);
            }
            return promise;
        };
    }
...
GovernanceAPIClient.spec = null;

export default GovernanceAPIClient;
